...
@Component({
  templateUrl: './verificationComponentHtml.html'
})
export class verificationComponent extends AppComponentBase
  implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userVerfication.userVerification(orderId, token).subscribe({
      ...
      error: (err) => {
        ...
        abp.notify.error(err.error?.message || 'Verification failed. Please try again.');
      }
    });
  }
}
