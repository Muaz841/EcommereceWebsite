...
import { ProductServiceProxy, CreateUpdateProductDto, ProductMediaDTO, 
  DiscountType,CategoryListDto, CategoryServiceServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class CreateProductComponent implements OnInit {
      ...
      if (this.productDto.id) {
        this._productService.getById(this.productDto.id).subscribe(
          (product) => {
            ...
            this.previewImages = product.images?.map((media) => `data:image/jpeg;base64,${media.image}`) || [];
  ...
  async submit(): Promise<void> {
    ...
    if (this.thumbnail) {
      this.productDto.thumbnail = await this.convertFileToBase64(this.thumbnail);
    }  
    this.productDto.images = await this.convertFilesToProductMediaDTO(this.images);    
    ...
    request.subscribe(
      () => {
        ...
        abp.notify.success(this.productDto.id ? 'Product updated successfully' : 'Product created successfully');  
  ...
  async convertFilesToProductMediaDTO(files: File[]): Promise<ProductMediaDTO[]> {
    return Promise.all(
